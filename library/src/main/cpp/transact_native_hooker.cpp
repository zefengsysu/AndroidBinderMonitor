...
#define TAG "AndroidBinderMonitor.TransactNativeHooker"
...
static jboolean hijackedTransactNative(JNIEnv*, jobject, jint, jobject, jobject, jint);

bool g_has_try_init = false;
bool g_init_success = false;
GetMethodShorty g_get_method_shorty;
GetNativeMethodCount g_get_native_method_count;
GetNativeMethods g_get_native_methods;
...
bool initIfNeed(JNIEnv *env) {
    ...
    auto art_handle = xdl_open("libart.so", XDL_DEFAULT);
    if (nullptr == art_handle) {
        LOGE(TAG, "dlopen art fail");
        return g_init_success;
    }
    g_get_method_shorty = (GetMethodShorty) xdl_dsym(art_handle, "_ZN3artL15GetMethodShortyEP7_JNIEnvP10_jmethodID", nullptr);
    if (nullptr == g_get_method_shorty) {
        LOGE(TAG, "dlsym GetMethodShorty fail");
        return g_init_success;
    }
    g_get_native_method_count = (GetNativeMethodCount) xdl_dsym(art_handle, "_ZN3artL20GetNativeMethodCountEP7_JNIEnvP7_jclass", nullptr);
    if (nullptr == g_get_native_method_count) {
        LOGE(TAG, "dlsym GetNativeMethodCount fail");
        return g_init_success;
    }
    g_get_native_methods = (GetNativeMethods) xdl_dsym(art_handle, "_ZN3artL16GetNativeMethodsEP7_JNIEnvP7_jclassP15JNINativeMethodj", nullptr);
    ...
    auto transact_native_method = env->GetMethodID(g_binder_proxy_class, kTransactNativeName, kTransactNativeSig);
    ...
    auto got_binder_proxy_native_method_count = g_get_native_methods(env, g_binder_proxy_class, native_methods, binder_proxy_native_method_count);
    ...
    g_hooker_class = env->FindClass("com/zack/monitor/binder/TransactNativeHooker");
    g_hooker_class = (jclass) env->NewGlobalRef(g_hooker_class);
    g_on_transact_start_method = env->GetStaticMethodID(g_hooker_class, "onTransactStart", "(Landroid/os/IBinder;ILandroid/os/Parcel;I)V");
...
bool TransactNativeHooker::hook(JNIEnv *env) {
    if (!initIfNeed(env)) {
        return false;
    }
    auto register_ret = env->RegisterNatives(g_binder_proxy_class, g_hijacked_binder_proxy_methods, NELEM(g_hijacked_binder_proxy_methods));
...
bool TransactNativeHooker::unhook(JNIEnv *env) {
    if (!g_init_success) {
        return true;
    }
    auto register_ret = env->RegisterNatives(g_binder_proxy_class, g_origin_binder_proxy_methods, NELEM(g_origin_binder_proxy_methods));
    if (JNI_OK != register_ret) {
...
static jboolean hijackedTransactNative(
    JNIEnv* env, jobject obj,
    jint code, jobject dataObj, jobject replyObj, jint flags
) {
    env->CallStaticVoidMethod(g_hooker_class, g_on_transact_start_method, obj, code, dataObj, flags);
